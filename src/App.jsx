...
function App() {
  ...
  const handleNext = () => {
    setSelectedIdx((prev) => Math.min(pokemons.length - 1, prev + 1));
  };

  return (
    <div className="p-3 md:w-[35%] mx-auto my-8">
      <SelectCard
        pokemons={pokemons}
        selectedIdx={selectedIdx}
        onSelect={handleSelect}
      />
      <div className="shadow-md border-2 rounded-md mt-4">
        <img
          className="w-full object-cover"
          src={pokemons[selectedIdx].image}
          alt="pokemon"
        />
        <h2 className="text-center my-3 font-extrabold text-2xl">
          {pokemons[selectedIdx].name?.charAt(0).toUpperCase() +
            pokemons[selectedIdx].name?.slice(1)}
        </h2>
        <p className="text-center text-base px-4 py-4">
          {pokemons[selectedIdx].description}
        </p>
      </div>
      <Pagination
        selectedIdx={selectedIdx}
        dataLen={pokemons.length}
